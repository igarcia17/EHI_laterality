...
if(oldies){
  ...
  config_df <- config_df%>%filter(has_RNA=="YES")%>%droplevels()
}

sapply(config_df, class)

#Load genetic expression data
files <- config_df$File

files <- paste0(filesF, config_df$File)
...
#Originally 63140 genes
keep <- rowSums(expr_matrix > 10) > (ncol(expr_matrix) * 0.9)
df <- expr_matrix[keep,] #drops to 14155
rm(keep,expr_matrix) #Remove because big size

#Normalize
dds <- DESeq2::DESeqDataSetFromMatrix(countData = df,
                              colData = config_df,
                              design = ~ 1) #to avoid design
dds_norm <- DESeq2::vst(dds, blind = TRUE)
df_norm <-  SummarizedExperiment::assay(dds_norm)
df_norm <- limma::removeBatchEffect(df_norm, batch=dds_norm$WAVE, design = matrix(1, ncol(df_norm), 1))
...
clust <- cutreeStatic(sampleTree, cutHeight = 111, minSize = 1)
tab <- table(clust)
...
#Check overall adjacency
A <- adjacency(t(datExpr), type = "signed") #connectivity of samples to each other
k <- apply(A, 2, sum) - 1#sum of each sample
Z.k <- scale(k) #Z score for normalization
outliers <- which(Z.k < -5)

jpeg(file = contreeF, width = 1700, height = 1700)
plot(Z.k,
     ylab = "Standardized connectivity (Z.k)",
     xlab = "Samples")
...
text(
  x = outliers,
  y = Z.k[outliers],
  labels = rownames(datExpr)[outliers],
  pos = 3,
  cex = 0.7,
  col = "red"
)
...
suffix <- gsub("[^0-9]", "_", as.character(Sys.Date()))
outF <- "WGCNA_blood/"
res_step1F <- paste0(outF, "data_step1_2026_05_27.RData")
load(res_step1F)

sign <- T
run_scale_free_topology <- F
